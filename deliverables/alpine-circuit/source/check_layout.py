...
for i,a in enumerate(p):
 ...
 for j in range(i+40,len(p)):
  ...
  if 0<=t<1 and 0<=u<1:
   za=a[2]+t*(b[2]-a[2]);zb=c[2]+u*(d[2]-c[2]);crossings.append({'xy':[round((a[0]+t*ux)*.25,3),round((a[1]+t*uy)*.25,3)],'deck_separation_m':round(abs(za-zb)*.25,3)})
print(json.dumps({'close_road_conflicts':len(conflicts),'crossings':crossings},indent=2))
