...
groups={}
for name in ['01_Plinth','02_Track','03_Terrain','04_Trees_and_Shrubs','05_Safety_Barriers','06_Clear_Catch_Fencing','07_Curved_LED_Tunnel','08_Screens','09_Floodlights','10_Collision','11_Studio','12_Grandstands_and_Crowds']:
...
cube('Black rectangular display plinth',(0,0,-.20),(13.8,17.4,.45),plinth,G[0],.12)
cube('Inset landscape tray',(0,0,.025),(13.52,17.10,.07),grass,G[2],.08)
cube('Plinth brushed reveal',(0,0,-.06),(13.75,17.35,.025),metal,G[0],.035)
...
for attempt in range(3800):
 ...
 if (min(np.linalg.norm(P[224:305,:2]-[x,y],axis=1))<1.0) or dist<.82 or any((x-a)**2+(y-b)**2<.27 for a,b in placed):continue
...
# Subtle satellite terrain, deliberately low to preserve circuit readability.
for k,(x,y,sx,sy) in enumerate([(-5.65,5.9,.7,1.1),(5.6,5.1,.6,1.0),(-.2,3.7,.5,.4),(.2,-4.6,.7,.5),(5.6,-6.9,.65,.65),(-5.6,-5.8,.65,.8)]):
 ico('Low meadow hill %02d'%k,(x,y,.07),(sx,sy,.18),grass,G[2],3)
 for j in range(3):
  ico('Small hillside stone %02d_%02d'%(k,j),(x+random.uniform(-.35,.35),y+random.uniform(-.4,.4),.12),(.12,.10,.095),rock,G[2],1)
# Two large, double-sided Jumbotrons with abstract racing graphics.
for k,(x,y) in enumerate([(-5.65,3.0),(5.65,-3.4)]):
 for dx in [-.68,.68]:
  cyl('Jumbotron %d structural mast'% (k+1),(x+dx,y,.06),(x+dx,y,1.95),.055,metal,G[7])
 cube('Jumbotron %d large enclosure'%(k+1),(x,y,2.08),(1.95,.16,1.03),black,G[7],.045)
 for side in [-1,1]:
  cube('Jumbotron %d display face'%(k+1),(x,y+side*.09,2.08),(1.82,.018,.90),screen,G[7])
  for j in range(12):
   h=.12+.42*(.5+.5*math.sin(j*.63+k))
   cube('Jumbotron abstract telemetry',(x-.80+j*.145,y+side*.104,1.78+h/2),(.072,.009,h),cyan if j%4 else led,G[7])
  cube('Jumbotron header ribbon',(x,y+side*.107,2.44),(1.65,.008,.025),white,G[7])
# Multiple terraced spectator sections: real linked miniature seated people.
seatmat=mat('Grandstand • muted blue seats',(.045,.12,.18),.48)
concrete=mat('Grandstand • light concrete',(.40,.43,.43),.85,texture=True)
skin=mat('Crowd • warm neutral skin',(.52,.30,.18),.8)
shirts=[mat('Crowd clothing '+str(k),c,.83) for k,c in enumerate([(.55,.035,.035),(.09,.19,.29),(.72,.67,.50),(.04,.065,.08),(.25,.31,.17)])]
people=[]
for k,m in enumerate(shirts):
 parts=[cube('Seated torso',(0,0,.105),(.072,.055,.10),m,G[11],.012),ico('Spectator head',(0,-.003,.192),(.035,.032,.040),skin,G[11],2)]
 for dx in [-.022,.022]:
  parts.append(cyl('Seated leg',(dx,-.01,.075),(dx,-.07,.048),.014,black,G[11],6))
  parts.append(cyl('Seated shin',(dx,-.07,.048),(dx,-.07,.005),.013,black,G[11],6))
  parts.append(cyl('Spectator arm',(dx*1.8,0,.14),(dx*1.8,-.04,.085),.011,m,G[11],6))
 people.append(join(parts,'Spectator reusable asset '+str(k)))
seat=cube('Seat reusable asset',(0,0,0),(.105,.10,.025),seatmat,G[11],.012)
# Stands face inward toward the course, with aisle breaks and safety rails.
stand_specs=[(-5.65,-2.8,-math.pi/2),(-5.65,.15,-math.pi/2),(5.65,1.9,math.pi/2),(5.65,-.7,math.pi/2)]
crowd_count=0
for si,(cx,cy,ang) in enumerate(stand_specs):
 def sw(x,y,z):return (cx+x*math.cos(ang)-y*math.sin(ang),cy+x*math.sin(ang)+y*math.cos(ang),z)
 def sc(name,xyz,dims,m):
  o=cube('Stand %d '%(si+1)+name,sw(*xyz),dims,m,G[11],.008);o.rotation_euler.z=ang;return o
 for row in range(5):
  yy=-.42+row*.19; zz=.10+row*.14
  sc('terrace row %d'%row,(0,yy,zz/2),(2.15,.19,zz),concrete)
  for col in range(16):
   xx=-.98+col*.13
   if col in [7,8]:continue
   loc=sw(xx,yy,zz+.028)
   o=bpy.data.objects.new('Stand %d blue seat'%(si+1),seat.data);groups[G[11]][0].objects.link(o);o.parent=groups[G[11]][1];o.location=loc;o.rotation_euler.z=ang
   proto=people[random.randrange(len(people))];o=bpy.data.objects.new('Stand %d spectator %03d'%(si+1,crowd_count),proto.data);groups[G[11]][0].objects.link(o);o.parent=groups[G[11]][1];o.location=loc;o.rotation_euler.z=ang+random.uniform(-.12,.12);crowd_count+=1
 for x in [-1.10,1.10]:
  for yy,zz in [(-.5,.4),(.5,1.05)]:cyl('Stand guardrail upright',sw(x,yy,zz-.3),sw(x,yy,zz),.015,metal,G[11],6)
  cyl('Stand raked side handrail',sw(x,-.5,.4),sw(x,.5,1.05),.017,metal,G[11],6)
 cyl('Stand rear safety handrail',sw(-1.1,.5,1.05),sw(1.1,.5,1.05),.017,metal,G[11],6)
for o in people+[seat]:bpy.data.objects.remove(o,do_unlink=True)
...
# Bake object rotation before the anisotropic layout expansion.
# This keeps barrier runs, railings and crowd geometry aligned after widening.
for o in list(bpy.data.objects):
 if o.type=='MESH' and any(abs(a)>1e-8 for a in o.rotation_euler):
  rot=o.rotation_euler.to_matrix().to_4x4(); o.data=o.data.copy();o.data.transform(rot);o.rotation_euler=(0,0,0)
...
for o in bpy.data.objects:
 if o.type=='MESH':
  ...
  o.location.x*=.35; o.location.y*=.4125; o.location.z*=.25
for me in bpy.data.meshes:
 for v in me.vertices: v.co.x*=.35; v.co.y*=.4125; v.co.z*=.25
scene=bpy.context.scene; scene.unit_settings.system='METRIC'; scene.unit_settings.scale_length=1
root['dimensions_m']='4.830 x 7.1775'; root['design']='Alpine Circuit Diorama'; root['reference']='IMG_6818.jpeg course; TrackTunnel.jpg; RailingsRef.jpg'; root['scale']='Physical miniature, metres'; root['no_paid_generation']=True
...
cam=camera('Overview • high three-quarter', (8,-11,13),(0,0,.05),9.5); scene.camera=cam
camera('Overview • opposite three-quarter',(-8,11,12),(0,0,.1),9.5)
camera('Blueprint • overhead',(0,0,12),(0,0,0),8.0)
...
tunnelcam=camera('Tunnel • entry detail',(1.22,-1.65,.17),(1.32,-.80,.25),1.15)
...
report={'mesh_objects':sum(o.type=='MESH' for o in bpy.data.objects),'unique_meshes':len(bpy.data.meshes),'triangles_unique':sum(sum(len(p.vertices)-2 for p in m.polygons) for m in bpy.data.meshes),'tree_instances':len(placed)+5,'materials':len(bpy.data.materials),'uv_missing':[o.name for o in bpy.data.objects if o.type=='MESH' and not o.data.uv_layers],'track_length_m':float(np.linalg.norm(np.diff(np.vstack([P,P[0]])*np.array([.35,.4125,.25]),axis=0),axis=1).sum()),'track_width_min_m':.266,'track_width_max_m':.3135,'spectators':crowd_count,'grandstand_sections':4,'jumbotrons':2,'tunnel_length_m':float(np.linalg.norm(np.diff(P[tids]*np.array([.35,.4125,.25]),axis=0),axis=1).sum())}
